"""SQLite metadata models for the Quant research platform."""
...
class MarketDataset(TimestampedRecord):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["name", "source"],
                name="uniq_quant_market_dataset",
            )
        ]
...
class Experiment(ReproducibleRecord, TimestampedRecord):
    """A reproducible Quant experiment envelope.
...
class QuantRunRecord(ReproducibleRecord, TimestampedRecord):
    """Shared fields for Quant research runs.
...
class RegimeRun(QuantRunRecord):
    """A regime detector run."""


class RiskRun(QuantRunRecord):
    """A risk model run."""


class LOBRun(QuantRunRecord):
    """A limit-order-book research run."""


class GraphSnapshot(QuantRunRecord):
...
class PortfolioRun(QuantRunRecord):
...
class ModelRun(QuantRunRecord):
    """A registered model run."""


class BacktestRun(QuantRunRecord):
    """A backtest simulation run."""


class ExplainabilityReport(QuantRunRecord):
