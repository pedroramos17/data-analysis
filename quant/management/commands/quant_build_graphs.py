"""Build Quant graph snapshots from local JSON series."""
...
from quant.services.graphs.graph_builders import (
...
class Command(BaseCommand):
    """Persist a local Quant graph snapshot."""

    help = "Build leakage-safe Quant graph snapshots from local JSON series."

    def add_arguments(self, parser: CommandParser) -> None:
        """Register graph build options."""
        parser.add_argument("--name", default="quant-graph")
        parser.add_argument("--series-json", required=True)
        parser.add_argument("--window-end", required=True)
        parser.add_argument("--output-dir", default="data/quant_graphs")
    ...
    def handle(self, *args: object, **options: object) -> None:
        ...
        snapshot = persist_graph_snapshot(
            ...
            provenance={"command": "quant_build_graphs"},
        )
