...
a Django public-source monitor, a Quant4 research app, a QuantSpace research-paper
...
| Quant4 | `quant4/` | Django app and services for assets, market datasets, feature artifacts, regimes, risk, graphs, LOB, portfolio, and experiments. |
| QuantSpace | `quantspace/` | Django app for research papers, chunks, citations, evidence, vector search, extraction, paper QA, and factor candidates. |
...
| Django management commands | `monitoring/management/commands/`, `quant4/management/commands/`, `quantspace/management/commands/` | Existing commands cover RSS/API/source ingestion, enrichment, clustering, comparison, graph/RAG context, finance exports, Quant4 experiments, market/LOB ingestion, paper ingestion, paper index building, and factor generation. |
| Django UI | `monitoring/urls.py`, `monitoring/views.py`, `monitoring/intelligence_views.py`, `quantspace/urls.py`, `quantspace/views.py` | Existing server-rendered screens and internal JSON endpoints for monitoring dashboards, digest, export rows, intelligence rows, and paper workflows. |
...
| Media files | `media/`, `quantspace.models.Paper.pdf_file` | Raw snapshots and uploaded research PDFs. |
...
| Manual/local files | `sourceflow/finance_ingestion/connectors/local_files.py`, `quant4/management/commands/quant4_import_bars.py`, `monitoring/management/commands/import_financial_dataset.py` | CSV, JSONL, and Parquet local imports. |
...
| Research PDFs | `quantspace/management/commands/ingest_paper_pdf.py`, `quantspace/services/pdf_extraction.py` | PDF ingestion into `Paper`, `PaperChunk`, citations, and extraction artifacts. |
...
| Finance/market data | `monitoring.MarketInstrument`, `MarketBar`, `MarketTick`, `quant4.Asset`, `MarketDataset`, `FeatureArtifact`, `RegimeRun`, `RiskRun`, `GraphSnapshot`, `PortfolioRun`, `ModelRun`, `BacktestRun` | Good base for assets, instruments, runs, risk, regimes, and graph snapshots; target plan needs risk graph and portfolio explanation contracts. |
| Research evidence | `quantspace.Paper`, `PaperChunk`, `PaperCitation`, `PaperQuestion`, `QuantExtraction`, `FactorCandidate` | Good provenance pattern for papers; target plan should reuse this evidence-first pattern for news and KG evidence. |
...
| Graph features | `sourceflow/finance_graph/graph_features.py`, `quant4/services/graphs/` | Existing graph feature and topology code is research-oriented, not a persistent KG store. |
| Article embeddings | `monitoring.ArticleEmbedding`, `quantspace/services/vector_search.py` | Local vector-like retrieval exists for articles/papers but is not yet a unified GraphRAG layer. |
| Existing RAG context | `monitoring/management/commands/build_graphrag_context.py`, `sourceflow/intelligence/xai/rag_context.py`, `quantspace/services/ask_paper.py` | Retrieval and explanation helpers exist, but proof-carrying GraphRAG with contradictions and KG paths is not yet formalized. |
...
Existing ingestion, Django models, Quant4 services, QuantSpace services, CLI/API,
