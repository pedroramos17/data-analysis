"""QuantSpace local research cockpit tests."""
...
class QuantSpaceServiceTests(TestCase):
    """Core QuantSpace services must work without paid APIs."""

    def test_chunking_preserves_page_ranges(self) -> None:
        """Merged chunks keep the first and last page numbers."""
        from quantspace.services.chunking import chunk_page_texts
    ...
    def test_vector_fallback_returns_relevant_chunks(self) -> None:
        ...
        from quantspace.models import Paper, PaperChunk
    ...
    @override_settings(SOURCEFLOW_FEATURE_FLAGS={"QUANTSPACE_LLM_PROVIDER": False})
    def test_ask_paper_returns_prompt_preview_without_llm(self) -> None:
        ...
        from quantspace.models import Paper, PaperChunk
        from quantspace.services.ask_paper import answer_paper_question
    ...
    def test_quant_extraction_parser_accepts_partial_json(self) -> None:
        ...
        from quantspace.services.quant_schema import parse_quant_extraction_payload
    ...
    def test_no_profitability_claims_in_generated_prompts(self) -> None:
        ...
        from quantspace.models import FactorCandidate, Paper
        from quantspace.services.prompt_export import build_codex_prompt
    ...
    @override_settings(
        SOURCEFLOW_FEATURE_FLAGS={"QUANTSPACE_PYMUPDF_EXTRACTION": False}
    )
    def test_feature_flags_block_disabled_optional_features(self) -> None:
        ...
        from quantspace.services.pdf_extraction import extract_pdf_pages
        from sourceflow.config.feature_flags import FeatureDisabledError

        with self.assertRaisesRegex(FeatureDisabledError, "QUANTSPACE_PYMUPDF"):
            extract_pdf_pages("paper.pdf")


class QuantSpacePageTests(TestCase):
    """Core QuantSpace pages should render in the local Django app."""
    ...
    def test_pages_render_locally(self) -> None:
        ...
        urls = [
            reverse("quantspace:paper-list"),
            reverse("quantspace:paper-upload"),
            reverse("quantspace:paper-detail", args=[paper.pk]),
            reverse("quantspace:paper-ask", args=[paper.pk]),
            reverse("quantspace:paper-extract", args=[paper.pk]),
            reverse("quantspace:factor-lab"),
        ]
