"""Tiny GLC/GNN smoke experiment with optional graph libraries."""
...
def _graph_or_fallback_metrics(dataset: dict[str, object]) -> dict[str, object]:
    try:
        import torch
...
def _write_metrics(config: dict[str, object], metrics: dict[str, object]) -> Path:
    output_dir = Path(str(config.get("output_dir", "exports/experiments/glc_gnn")))
