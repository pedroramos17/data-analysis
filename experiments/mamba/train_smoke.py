"""Tiny Mamba-like smoke experiment with optional PyTorch."""
...
def main() -> int:
    ...
    metrics = _torch_or_fallback_metrics(dataset, config)
...
def _torch_or_fallback_metrics(
    dataset: dict[str, object], config: dict[str, object]
) -> dict[str, object]:
    try:
        import torch
    except ImportError:
        return _fallback_metrics(dataset, config, "numpy_fallback")
    samples = torch.as_tensor(dataset["samples"]).float()
    targets = torch.as_tensor(dataset["targets"]).float()
    loss = torch.mean((samples[:, -1, :] - targets) ** 2).item()
    return {"loss": loss, "backend": "torch", "samples": int(samples.shape[0])}


def _fallback_metrics(
    dataset: dict[str, object], config: dict[str, object], backend: str
) -> dict[str, object]:
    samples = dataset["samples"]
    targets = dataset["targets"]
    loss = float(((samples[:, -1, :] - targets) ** 2).mean())
...
def _write_metrics(config: dict[str, object], metrics: dict[str, object]) -> Path:
    output_dir = Path(str(config.get("output_dir", "exports/experiments/mamba")))
