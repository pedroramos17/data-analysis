"""Export ResearchSpace chunks to local Parquet artifacts."""
...
def export_paper_chunks_to_parquet(paper: object) -> object:
    ...
    """
    require_feature("RESEARCHSPACE_PARQUET_EXPORT")
    try:
        import pyarrow as pa
        import pyarrow.parquet as pq
    except ImportError as error:
        raise RuntimeError(
            "pyarrow is required for ResearchSpace Parquet export"
        ) from error
...
def _chunk_export_path(paper: object) -> Path:
    ...
    return root / "researchspace" / f"paper_{paper.id}_chunks.parquet"


def _create_artifact(paper: object, path: Path) -> object:
    from researchspace.models import PaperArtifact
