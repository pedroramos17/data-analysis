"""Run the safe Quant4 full-experiment orchestrator."""
...
from quant4.services.full_experiment import (
...
class Command(BaseCommand):
    """Plan or run a safe local Quant4 experiment DAG."""

    help = "Plan or run the safe local Quant4 full-experiment DAG."
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument("--data-root", default="data/quant4")
    ...
    def handle(self, *args: object, **options: object) -> None:
        ...
        self.stdout.write(f"Quant4 DAG: {dag_summary()}")
...
def _can_skip_persistence(config: FullExperimentConfig, exc: OperationalError) -> bool:
    return config.dry_run and "quant4_experiment" in str(exc)


def _persistence_skip_message(exc: OperationalError) -> str:
    return (
        f"experiment_persistence=skipped reason={str(exc)!r}; "
        "expected migrated Quant4 database for Experiment records"
    )
