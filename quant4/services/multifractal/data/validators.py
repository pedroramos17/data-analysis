...
from quant4.services.multifractal.data.contracts import OHLCVBar, ReturnRecord
...
def _bar_from_csv_row(
    ...
    return OHLCVBar(
        ...
        _positive_float(row.get("open"), "open"),
        _positive_float(row.get("high"), "high"),
        _positive_float(row.get("low"), "low"),
        _positive_float(row.get("close"), "close"),
        _optional_float(row.get("volume"), "volume"),
        _optional_text(row.get("currency")),
        source,
        timeframe,
        _optional_positive_float(row.get("adjusted_close"), "adjusted_close"),
    )
